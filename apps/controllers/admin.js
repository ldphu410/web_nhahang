...
var doanhthu_md = require('../models/doanhthu')
...
router.post("/admin_signout", (req, res) => {
    var param = req.body
...
router.get("/datban/in", (req, res) => {
    if (req.session.admin) {
        ...
        datban.then((dt) => {
            ...
            res.render("admin/xuly/print", { data: data })
        })
    } else {
        res.redirect("/admin/admin_signin")
    }
...
router.get("/doanhthu", (req, res) => {
    if (req.session.admin) {
    var data = doanhthu_md.getAllDoanhThu()
    data.then((dt) => {
        var tong = {
            t1: 0,
            t2: 0,
        ...
        res.render("admin/xuly/trangdoanhthu", { doanhthu: dt, tong: tong, error: null })
    })
    } else {
        res.redirect("/admin/admin_signin")
    }
...
router.get("/doanhthu/them", (req, res) => {
     if (req.session.admin) {
    res.render("admin/xuly/doanhthu")
     } else {
        res.redirect("/admin/admin_signin")
    }
})

router.post("/doanhthu/them", (req, res) => {
    var data = req.body
    var params = {
        ngay: data.ngay,
        thu1: data.thu1,
    ...
    var add = doanhthu_md.addDoanhThu(params)
    add.then((dt) => {
        res.redirect("/admin/doanhthu")
    })

})

router.post("/doanhthu/loc", (req, res) => {
    var param = req.body
    var n1 = param.ngay1
    var n2 = param.ngay2
    ...
    if (!param.ngay1 || !param.ngay2) {
        ...
        data.then((dt) => {
            ...
            res.render("admin/xuly/trangdoanhthu", { doanhthu: dt, tong: tong, error: "Cần chọn đủ 2 mốc thời gian" })
        })
    } else {
        var rr1 = doanhthu_md.getDoanhThuByDate(n1)
        rr1.then((dt) => {
            id1 = dt[0].id
            var rr2 = doanhthu_md.getDoanhThuByDate(n2)
            rr2.then((dt1) => {
                id2 = dt1[0].id
                var data = doanhthu_md.getDS(id1 - 1, id2-id1+1)
                data.then((dt2) => {
                    var tong = {
                        t1: 0,
                        t2: 0,
                    ...
                    res.render("admin/xuly/trangdoanhthu", { doanhthu: dt2, tong: tong, error: null })
                })
            })
